...
module.exports = {
  ...
  build: {
    ...
    preflight: {
      ...
      prettier: false // 构建时是否进行prettier检测 (whether or not process prettier checking)
    }
  },
  ...
  release: {
    ...
    git: 'git@github.com:omni-door/gulp-plugin-vue-sfc.git',
    ...
    preflight: {
      ...
      prettier: false, // 发布前是否进行prettier检测 (whether or not process prettier checking)
      commitlint: false, // 发布前是否进行commitlint检测 (whether or not process commitlint checking)
      branch: 'main' // 发布前进行分支检测，设置为空字符串则不会检测 (only can release in this branch, set empty string to ignore this check)
    }
  },
